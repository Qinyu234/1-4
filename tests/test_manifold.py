"""Manifold: linear polys + Td-symmetric velocity kicks + COM frame."""
...
def test_theta_roundtrip():
    p = ManifoldParams(a0=1.0, a1=0.1, e0=0.05, e1=0.01, M0=0.0, M1=0.3, vx=0.01, vy=-0.02, vz=0.0, mu_mass=1e-3)
    assert ManifoldParams.from_theta(p.as_theta()) == p
    assert len(p.as_theta()) == 10
...
def test_from_error_seed_anchors():
    p = from_error_seed(1e-3, 0.05, a1=0.12, e1=0.01, M1=2.0, vx=0.01)
